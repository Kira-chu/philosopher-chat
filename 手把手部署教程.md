...
**情况 A：如果你看到部署选项列表（GitHub Repository、Database 等）**

1. 在部署选项列表中，找到并点击 **"GitHub Repository"**（GitHub 仓库）
   - **注意**：不要点击 "Database"、"Template" 等其他选项
   - 应该能看到一个 GitHub 的猫猫图标
2. 如果提示授权 GitHub，点击 **"Configure GitHub App"** 或 **"Authorize"** 并授权
3. 授权后，会显示你的 GitHub 仓库列表
4. 在列表中找到你的 `philosopher-chat` 仓库
5. 点击仓库名称

**情况 B：如果你已经创建了项目，看到 "Add a Service" 界面**

1. 在页面中央的虚线框内，点击 **"Add a Service"**（添加服务）
   - 或者点击右上角的 **"+ Create"** 按钮
   - 或者按快捷键 **⌘K**（Mac）或 **Ctrl+K**（Windows）
2. 在弹出的菜单中，选择 **"GitHub Repo"** 或 **"Deploy from GitHub repo"**
3. 如果提示授权 GitHub，点击授权
4. 授权后，会显示你的 GitHub 仓库列表
5. 在列表中找到你的 `philosopher-chat` 仓库
6. 点击仓库名称

### 2.3 配置服务（重要！）

**Root Directory 在服务设置中，不在项目设置中！**

1. 在项目页面，点击左侧的 **"philosopher-chat"** 服务卡片（带 GitHub 图标）
2. 进入服务详情页面后，点击顶部的 **"Settings"**（设置）标签
3. 在设置页面中，找到 **"Root Directory"**（根目录）部分
4. 点击 **"Edit"** 或输入框
5. 输入：`backend`
6. 点击 **"Save"**（保存）

**如果还是找不到 Root Directory：**

1. 点击左侧的 **"philosopher-chat"** 服务卡片
2. 在服务详情页面，找到 **"Source"**（源代码）部分
3. 点击 **"Edit"** 或 **"Configure"**
4. 在配置中查找 **"Root Directory"** 或 **"Working Directory"**
5. 设置为：`backend`

**重要提示：**
- Root Directory 必须在服务（Service）的设置中，不是在项目（Project）的设置中
- 如果 Root Directory 没有设置为 `backend`，Railway 会找不到 Python 文件
- 设置后，Railway 会自动重新部署
